...
pid_ptr new_pid_node(int pid, int index){
...
void pid_hash_insert(PIDHASH records, int pid, int index){
  ...
  pid_hash_list_insert(&(records[h].start), pid, index);
}

//Inserts a new pid in the list of the hash node
void pid_hash_list_insert(pid_ptr* head, int pid, int index){
  if(*head == NULL){
    *head = new_pid_node(pid, index);
  }else{
    ...
    traverse->next = new_pid_node(pid, index);
  }
}
